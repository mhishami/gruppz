(function(){
  ...
  if( $.cookie( 'template_bg_grd' ) === undefined ) {
    $.cookie( 'template_bg_grd', false );
  } else{
    document.querySelector('#skins-grd-bg').checked = ($.cookie( 'template_bg_grd' ) === 'true'); // return of cookie is a string so we need to convert to boolean
    switcheryOnChange('#skins-grd-bg');
  }

  $( document ).on( 'change', '#skins-grd-bg', function(){
    // inject gradien to header and sidebar
    // so we need
    var whSkin = window.wh.options.skin,
    sidebarSkin = window.sidebar.options.skin,
    isChecked = $( this ).is(':checked');

    $.cookie( 'template_bg_grd', isChecked );

    // fire event (just tricky)
    window.wh.setSkin(whSkin);
    window.sidebar.setSkin(sidebarSkin);
  });
